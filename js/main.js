...
class Player{
    constructor(){
        ...
        sphere.position.z = 1
        sphere.position.x = start_position - .4
        scene.add( sphere )
        this.player = sphere
        this.playerInfo = {
...
function startGame(){
    ...
    gsap.to(progressBar.scale, {x: 0, duration: TIME_LIMIT, x: 0, ease: "none"})
    setTimeout(() => {
        if(gameStat != "over"){
            text.innerText = "Tempo esgotado!"
            gameStat = "over"
        }
    }, TIME_LIMIT * 1000);
    doll.start()
